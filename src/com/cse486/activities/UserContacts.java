...
import com.cse486.R;
import com.cse486.adapters.ContactsAdapter;
...
public class UserContacts extends Activity implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while (phones.moveToNext()) {
			...
			try {
				...
				ContactNo = phoneUtil.format(
						phoneUtil.parse(ContactNo, countryCode.toUpperCase()),
						PhoneNumberFormat.E164).replaceAll("[-+^]*", "");
		...
		ContactsAdapter objAdapter = new ContactsAdapter(UserContacts.this,
				R.layout.list_item, listContacts);
	...
	public void onItemClick(AdapterView<?> listView, View view, int position,
		...
		try {
			myNo = phoneUtil.format(
					phoneUtil.parse(myNo, countryCode.toUpperCase()),
					PhoneNumberFormat.E164).replaceAll("[-+^]*", "");
		} catch (NumberParseException e) {
			e.printStackTrace();
		}
		...
		Bundle bndlanimation = ActivityOptions.makeCustomAnimation(
				getApplicationContext(), R.anim.animation1, R.anim.animation2)
				.toBundle();
